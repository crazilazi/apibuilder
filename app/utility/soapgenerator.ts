...
//     let objects = {};
//     for (let operationName in this.operations) {
//         let operationObjects = this.operations[operationName].getObjects();
//         for (const objectName in operationObjects) {
...
//             const requestObject = new options.constructor();
//             requestArgument = requestArgument[0];
//             for (const requestObjectAttribute in requestArgument) {
//                 const requestAttribute = requestArgument[requestObjectAttribute][0]._;
...
//         case "number":
//             const requestInteger = Number(requestArgument);
...
//     for (const iKey in soapRequest) {
//         if (iKey.includes("Envelope")) {
//             envelopeElementName = iKey;
//             for (const jKey in soapRequest[envelopeElementName]) {
...
//     }
//     const operationRequest = soapRequest[envelopeElementName][bodyElementName][0];
//     for (const operationRequestName in operationRequest) {
//         console.log(`operation: ${operationRequestName} requested`);
//         console.log(JSON.stringify(operationRequest));
//         if (operationRequestName === "$") {
//             continue;
//         }
//         const operationName = operationRequestName.substr(operationRequestName.indexOf(":") + 1);
//         const requestArguments = operationRequest[operationRequestName][0];
//         delete requestArguments.$;
//         const operation = service.getOperation(operationName);
...
//         let inputNamePrefix = "";
//         const firstRequestArgumentKey = Object.keys(requestArguments)[0];
...
//         for (const inputName in operation.inputs) {
...
//             const operationResponsePromise = operation.exec.apply(operation, operationArguments);
...
//                 }
//                 ret += ">\n";
//                 for (let i = 0; i < js[node].length; i++) {
...
//                 const soapResponsePromise = This.handleOperationRequest(service, soapRequest);
...
//     if (urlParts[1] === "wsdl") {
//         const wsdl = this.getWsdl(req.headers.host, service);
...
//     let ret = indent + '<xsd:element name="' + name + '"  minOccurs="0">\n';
//     ret += indent + "	<xsd:complexType>\n";
//     ret += indent + "		<xsd:sequence>\n";

//     for (const memberName in options.members) {
...
//         throw new Error("getWsdl expect service or type SoapService");
//     }

//     let wsdl = '<?xml version="1.0" ?>\n';
//     wsdl += "	<definitions\n";
...
//     for (const operationName in service.operations) {
//         const operation = service.operations[operationName];
//         const operationObjects = service.operations[operationName].getObjects();
//         for (const objectName in operationObjects) {
//             // objects[objectName] = operationObjects[objectName];
//             const object = operationObjects[objectName];
//             wsdl += '			<xsd:element name="' + objectName + '" type="tns:' + objectName + '">\n';
//             wsdl += '			<xsd:complexType name="' + objectName + '">\n';
//             wsdl += "				<xsd:sequence>\n";

//             for (const memberName in object) {
...
//             for (const faultName of operation.faults) {
